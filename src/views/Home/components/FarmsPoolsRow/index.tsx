...
import { Flex, Box, SwapVertIcon, IconButton } from 'canaryx-uikit'
...
const FarmsPoolsRow = () => {
  ...
  const getPoolText = (pool: DeserializedPool) => {
    if (pool.isAutoVault) {
      return t('Auto CANARY')
    }

    if (pool.sousId === 0) {
      return t('Manual CANARY')
    }
  ...
  return (
    <div ref={observerRef}>
      <Flex flexDirection="column" mt="24px">
        <Flex mb="24px">
          <RowHeading text={showFarms ? t('Top Farms') : t('Top CANARY Pools')} />
          <IconButton
            variant="text"
            height="100%"
            width="auto"
